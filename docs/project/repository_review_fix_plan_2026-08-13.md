...
- [ ] 12. Разбить `run_simulation` (432 строки, ~25 локальных аккумуляторов) на
      ...
      сборка summary. **Не начато.** Отложено сознательно: самый рискованный
      пункт плана (imperative-функция с ~25 threaded локальными переменными в
      исследовательском коде, от которого зависят научные выводы) заслуживает
      отдельного прохода с построчной проверкой, а не последнего рывка в конце
      уже большой сессии.
- [ ] 13. Ослабить связность `belief_cluster_route`/`belief_orienteering`,
      ...
      юнит-тестировать один режим в изоляции от другого. **Не начато**, по той
      же причине, что и пункт 12 — требует аккуратного выделения общей
      "single-best-candidate" логики в отдельную функцию без потери поведения,
      не одношаговая правка.
...
Полный набор тестов после волны 3 (частичной): 161 passed, без изменений
численных результатов относительно волны 2.
