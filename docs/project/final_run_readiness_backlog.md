...
> **Историческая заметка (2026-08-13):** документ описывает состояние проекта на legacy `cleanup_sim`, до пивота на `cleanup_sim_v2`. Актуальный backlog готовности — `docs/project/simulator_v2_1_closure_report.md`. Не использовать как актуальный источник для статьи без сверки с `cleanup_sim_v2`.
...
- `P2-04/P2-05` закрыты инфраструктурно: добавлены `pyproject.toml`, GitHub Actions `tests.yml`, `.gitattributes` и `docs/project/reproducibility_notes.md`.
...
- `P2-07` закрыт preregistration-документом: `docs/article/stage2/final_confirmatory_preregistration.md` фиксирует arms, scenarios, seed-ы, бюджеты, метрики и правило интерпретации до финального запуска.
...
| P2-02 | Closed | world | Мусор статичен, нет течений/ветра | добавить future work или отдельный stress scenario | закрыто: static debris явно записан как baseline-допущение и future work |
...
| P2-07 | Closed | experiment | Candidate-v2 выбран на pilot | pre-register final config и final seeds | закрыто: final config/seeds/budgets/metrics заранее описаны; tuning-on-test запрещен |
