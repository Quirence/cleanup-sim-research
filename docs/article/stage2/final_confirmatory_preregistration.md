...
Статус на 2026-08-16: историческая preregistration для старой ветки
`cleanup_sim`/`hybrid_final_v1`. Не запускать как финальный протокол для
`cleanup_sim_v2` без пересборки arms, сценариев и правила интерпретации.
