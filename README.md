...
- `tests/` - regression/unit tests только для актуального `cleanup_sim_v2`; тесты v1 удалены из активного контура.
...
Для продолжения работы достаточно сначала прочитать [текущее состояние](docs/project/current_state.md), затем [единственный действующий план](docs/project/next_research_plan.md). Изменения коллеги из PR #4, результаты прогонов и ограничения их интерпретации включены в актуальную историю `main`.

Ближайший шаг: закрыть расхождение воспроизводимости, реализовать расчет режимных показателей и спроектировать второй слой чувствительности. Бюджет `3000 м` остается диагностическим кандидатом. Seed-ы `100-129` уже использованы в тяжелой calibration-серии и не являются нетронутой финальной выборкой.
