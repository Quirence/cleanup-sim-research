...
9. `out/cleanup_sim_v2/`, `out/cleanup_sim/`
   - локальные результаты симулятора; новые файлы здесь исключены из Git по умолчанию;
   - публикационно важные таблицы переносить в `docs/project/evidence/` либо добавлять принудительно только после проверки. Исторические tracked CSV не перезаписывать результатами новых версий.
